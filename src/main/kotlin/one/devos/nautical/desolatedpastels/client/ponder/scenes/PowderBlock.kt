...
object PowderBlock {
    fun powderIntro(scene: SceneBuilder, util: SceneBuildingUtil) {
        ...
        val emitter = scene.effects().simpleParticleEmitter(ParticleTypes.CLOUD, Vec3(0.0, 0.1, 0.0))
...
//        scene.addKeyframe()
        ...
        scene.overlay().showText(40).text("Place the pastel powder into the water").pointAt(Vec3(2.0, 2.75, 0.0))
...
//        scene.addKeyframe()
        ...
        scene.world().replaceBlocks(util.select().position(BlockPos(2, 1, 0)), SofterPastelsBlocks.WHITE_PASTEL_BLOCK.defaultBlockState(), false)
        scene.world().replaceBlocks(util.select().position(BlockPos(0, 1, 2)), SofterPastelsBlocks.WHITE_PASTEL_BLOCK.defaultBlockState(), false)
